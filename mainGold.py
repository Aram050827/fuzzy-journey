...
def display_card_as_text(card_id, grid, marked):
    if grid is None:
        return "❌ Քարտը սխալ է։ Խնդրում ենք կապվել աջակցության հետ։"
    ...
    if context.args and context.args[0].startswith("game_"):
        ...
        if str(user_id) in player_ids:
            await update.message.reply_text(
                ...
                "📜 Ստուգեք Ձեր քարտը կամ սպասեք խաղի մեկնարկին։",
                reply_markup=get_main_menu()
            )
        ...
        welcome_message = (
            f"👋 Բարև, {user.first_name}։ Ես Հայկական Լոտո բոտն եմ։ 🎲\n"
...
async def show_cards(context: ContextTypes.DEFAULT_TYPE, user_id, game_id):
    ...
    if not cards:
        await context.bot.send_message(
            user_id,
            "❌ Դուք քարտ չունեք։ Կապվեք աջակցության հետ՝ @LottoSupport։",
            reply_markup=get_main_menu()
        )
        return
    for card_id, numbers, marked_numbers, positions, _ in cards:
        ...
        if num_count != 15:
            await context.bot.send_message(
                user_id,
                f"❌ Քարտը (ID: {card_id[-8:]}) սխալ է։ Կապվեք աջակցության հետ՝ @LottoSupport։",
                reply_markup=get_main_menu()
            )
            continue
        try:
            ...
            if keyboard is None:
                await context.bot.send_message(
                    user_id,
                    f"❌ Քարտը (ID: {card_id[-8:]}) չի ցուցադրվում։ Կապվեք աջակցության հետ՝ @LottoSupport։",
                    reply_markup=get_main_menu()
                )
                continue
            await context.bot.send_message(
                ...
                if len(player_ids) < MIN_PLAYERS and status == 'running':
                    ...
                    for pid in player_ids:
                        try:
                            await context.bot.send_message(
                                pid,
                                "🏁 Խաղն ավարտվեց, քանի որ խաղացողների քանակը բավարար չէ։\n"
                                "🎮 Ստեղծեք նոր խաղ կամ միացեք այլ խաղի։",
                                reply_markup=get_main_menu()
                            )
                        except Exception as e:
...
async def handle_friends_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔗 Հրավիրեք Ձեր ընկերներին խաղին միանալու համար՝\n{invite_link}"
    )
    await show_cards(context, user_id, game_id)
